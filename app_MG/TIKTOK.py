...
async def progress_hook(d, context: CallbackContext, chat_id, message_id, total_bytes=None):
    try:
        if d['status'] == 'downloading':
            ...
            message = (
                f"📥 **در حال دانلود ویدیو از تیک‌تاک**\n\n"
...
# دانلود ویدیو از تیک تاک با نمایش پیشرفت
async def download_tiktok_video(update: Update, context: CallbackContext):
    ...
    try:
        ...
        if 'tiktok.com' not in url:
            await update.message.reply_text("❌ لینک ارسال شده معتبر نیست! لطفاً فقط لینک تیک‌تاک ارسال کنید.")
            return "WAITING_FOR_LINK"
        ...
        # نمایش منوی مجدد
        return await show_tiktok_menu(update, context)
...
# نمایش منوی تیک‌تاک
async def show_tiktok_menu(update: Update, context: CallbackContext):
    keyboard = [
        [InlineKeyboardButton("ارسال لینک", callback_data="tiktok_link")],
        [InlineKeyboardButton("بازگشت", callback_data="back_to_main")],
    ]
